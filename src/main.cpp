...
bool model_init()
{
    ...
    nse_series.name_variable(4, "Cs2");
    nse_series.name_variable(5, "Cs");
    nse_series.name_variable(6, "eddy visc Z dissipation");
    nse_series.name_variable(7, "ssm Z dissipation");
    nse_series.name_variable(8, "negvisc Z dissipation");
    nse_series.name_variable(9, "model Z dissipation");
    nse_series.name_variable(10, "eddy visc E dissipation");
    nse_series.name_variable(11, "ssm E dissipation");
    nse_series.name_variable(12, "negvisc E dissipation");
    nse_series.name_variable(13, "model E dissipation");
    nse_series.name_variable(14, "negative Esub percent");
    nse_series.name_variable(15, "mean Esub");
	nse_series.name_variable(16, "LM backscatter percent");
	nse_series.name_variable(17, "LM diss to back");
	nse_series.name_variable(18, "MSE germano");
	nse_series.name_variable(19, "corr germano");
	nse_series.name_variable(20, "dissipation ratio germano");
	nse_series.name_variable(21, "lag time");
	nse_series.name_variable(22, "Decay time of subgrid energy");
	nse_series.name_variable(23, "eddy visc E diss galil");
	nse_series.name_variable(24, "negvisc E diss galil");
	nse_series.name_variable(25, "Csim Reynolds");
...
bool advance_nse_eq_runge_kutta()
{
		...
		#ifdef DYNAMIC_MODEL
		dyn_model.update_viscosity(w, U, V, Psi, dt, false, (Real)0.0, grid);
		#endif
		...
		memcpy(phi_rk, phi, grid.size * sizeof(Real));
        ...
        for (int step = 0; step < max_step; step++)
        {
            ...
            #ifdef J1
                w_J1(wim, w_rk, Psi_rk, grid);
				w_J1(phim, phi_rk, Psi_rk, grid);
            #endif
                
            #ifdef J2
                w_J2(wim, w_rk, Psi_rk, grid);
				w_J2(phim, phi_rk, Psi_rk, grid);
            #endif
                
            #ifdef J3
                w_J3(wim, w_rk, Psi_rk, grid);
				w_J3(phim, phi_rk, Psi_rk, grid);
            #endif
                
            mul(wim, (Real)1.0 / (Real)NUM, grid.size);
			mul(phim, (Real)1.0 / (Real)NUM, grid.size);

            #ifdef DIFFUSION
            w_diffusion(wim, w_rk, (Real)1.0 / c_Reynolds, grid);
			w_diffusion(phim, phi_rk, (Real)1.0 / c_Reynolds, grid);
            #endif
			...
			assign(phi_rk, (Real)1.0, phi, q[step] * dt, phim, grid.size);
            w_bc(phi_rk, grid);
		...
		memcpy(phi, phi_rk, grid.size * sizeof(Real));
...
bool advance_time()
{
	...
	if ((grid.mpi_com.rank == 0) && (time_index % ndebug == 0)) {
		...
		printf(" >> velocity check [c-norm] = %.8f\n", 
			u_error_cnorm + v_error_cnorm);
        printf(" >> streamfunction check [c-norm] = %.8f\n", 
			psi_error_cnorm);
		printf(" >> vorticity check [c-norm] = %.8f\n", 
			w_error_cnorm);
		printf("Cback = %.4f, E = %.4f \n", dyn_model.Csim_back, balance.en);
	...
	nse_series.push(4,   (double)dyn_model.Cs2_mean);
    nse_series.push(5,   (double)dyn_model.Cs);
    nse_series.push(6,   (double)dyn_model.t_Z_mean);
    nse_series.push(7,   (double)dyn_model.ssm_Z_mean);
    nse_series.push(8,   (double)dyn_model.b_Z_mean);
    nse_series.push(9,   (double)dyn_model.model_Z_mean);
    nse_series.push(10,  (double)dyn_model.t_E_mean);
    nse_series.push(11,  (double)dyn_model.ssm_E_mean);
    nse_series.push(12,  (double)dyn_model.b_E_mean);
    nse_series.push(13,  (double)dyn_model.model_E_mean);
    nse_series.push(14,  (double)dyn_model.E_neg_p);
    nse_series.push(15,  (double)dyn_model.Esub_mean);   
	nse_series.push(16,  (double)dyn_model.LM_back_p);
	nse_series.push(17,  (double)dyn_model.LM_diss_to_back);
	nse_series.push(18,  (double)dyn_model.MSE_germano);
	nse_series.push(19,  (double)dyn_model.C_germano);
	nse_series.push(20,  (double)dyn_model.model_diss_to_l_diss_germano);
	nse_series.push(21,  (double)dyn_model.mean_lag_time);
	nse_series.push(22,  (double)dyn_model.Esub_time);
	nse_series.push(23,  (double)dyn_model.t_E_mean_flux);
	nse_series.push(24,  (double)dyn_model.b_E_mean_flux);
	nse_series.push(25,  (double)dyn_model.Csim_back);
	...
	while (current_time < end_time) {
		...
		if (current_time >= print_mark) {       
			...
			*/
			...
			write_binary_przgn(OUTPUT_DIR"phi.nsx", phi, grid, print_index);
			...
			// germano identity
			#ifdef DYNAMIC_MODEL
			write_binary_przgn(OUTPUT_DIR"lx.nsx", dyn_model.lx, grid, print_index);
			write_binary_przgn(OUTPUT_DIR"ly.nsx", dyn_model.ly, grid, print_index);
			write_binary_przgn(OUTPUT_DIR"mx.nsx", dyn_model.mx, grid, print_index);
			write_binary_przgn(OUTPUT_DIR"my.nsx", dyn_model.my, grid, print_index);
			write_binary_przgn(OUTPUT_DIR"hx.nsx", dyn_model.hx, grid, print_index);
			write_binary_przgn(OUTPUT_DIR"hy.nsx", dyn_model.hy, grid, print_index);
			write_binary_przgn(OUTPUT_DIR"Bx.nsx", dyn_model.Bx, grid, print_index);
			write_binary_przgn(OUTPUT_DIR"By.nsx", dyn_model.By, grid, print_index);
			...
			write_binary_przgn(OUTPUT_DIR"Ediss.nsx", dyn_model.Ediss, grid, print_index);
			write_binary_przgn(OUTPUT_DIR"Eback.nsx", dyn_model.Eback, grid, print_index);
			write_binary_przgn(OUTPUT_DIR"Esub.nsx", dyn_model.Esub, grid, print_index);
			write_binary_przgn(OUTPUT_DIR"nub.nsx", dyn_model.nub, grid, print_index);
			#endif
