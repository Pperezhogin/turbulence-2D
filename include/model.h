...
const int coarse_coef = domain_nx / 128;
...
//spectrum parameters
const Real CFL = (Real) 0.8 / (coarse_coef * coarse_coef); //CFL = 1 is a maximum
Real dt;
...
#else
const Real c_Reynolds = (Real) 40000;
#endif

// ----------------------------------------------------- //
// --- output parameters --- //
#define OUTPUT_DIR	           "output/ADM/order_100/" 

// --- dynamic model params --- //
Real tf_width = sqrt(6.0) * coarse_coef;
Real bf_width = sqrt(6.0) * coarse_coef;
...
Real u_max = (Real)6.0, v_max = (Real)6.0;				// maximum velocity values //
#endif
#ifdef JET_INSTABILITY
...
#ifdef DECAYING_TURBULENCE
	#ifdef START_TIME_1
	read_binary_przgn("psi_time1_128filter_256mesh.nsx", Psisol, grid);
	#else
